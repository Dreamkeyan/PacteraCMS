...
//            $auto = autoLogin($account.'|'.md5($password).'|'.$log_ip,1);
            cookie('uid',$user['user_id'],C('CACHE_TIME'));
            session('uid', $user['user_id']);
    ...
    public function register($data = array()) {
        ...
        if ($this->isuc) { //开启了UC
            ...
            if (!isset($data['is_verify'])) $data['is_verify'] = 1; //直接激活
            $data['user_id'] = $obj->add($data);
        }
    ...
    public function saveDefaultOrderTemp($uid, $buyer_id) {
        $data = D('BuyerQuickOrderTemp')->field('id,short_name')->where(array('user_id' => $uid, 'buyer_id' => $buyer_id))->find();
        if (empty($data) || $data['short_name'] != 'myGoods') {
            $temp['buyer_id'] = $buyer_id;
            $temp['user_id'] = $uid;
            $temp['create_time'] = NOW_TIME;
            $temp['is_default'] = 1;
            $temp['name'] = '我的商品库';
            $temp['short_name'] = 'myGoods';
            D('BuyerQuickOrderTemp')->where(array('user_id' => $uid, 'buyer_id' => $buyer_id))->add($temp);
        }
    }
